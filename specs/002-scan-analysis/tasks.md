...
- [ ] T002 Xác nhận contract public API `/products/extract` và envelope trước khi chỉnh upload/decode flow.
...
- [ ] T007 Rà soát `NutriQuor/NutriQuor/Models/Product.swift`, `NutriQuor/NutriQuor/Models/APIResponse.swift` và `NutriQuor/NutriQuor/Models/DTOs/AnalyzeResponse.swift`.
- [ ] T008 Rà soát các file scan preview/upload trong `NutriQuor/NutriQuor/Features/ScanNutri`.
...
## Phase 4: User Story 2 - Upload Ảnh Và Bàn Giao Kết Quả (P1)

**Goal**: App upload ảnh đã chọn đến Backend API, decode `Product` và bàn giao cho feature hiển thị kết quả.

**Independent Test**: Dùng dev backend hoặc giả lập thành công/lỗi response để kiểm tra UI đang xử lý, lỗi và chuyển sang màn result khi thành công.
...
- [ ] T014 [US2] Kiểm tra callback/navigation bàn giao `Product` sang `006-analysis-result-display`.
- [ ] T015 [US2] Kiểm tra feature này không tự render chi tiết product result hoặc detail navigation.
...
- [ ] T019 Xác nhận thủ công hành vi xem trước ảnh, upload, đang xử lý, lỗi và chuyển sang màn result với người phát triển.
